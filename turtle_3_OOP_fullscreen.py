...
class MyTurtle(Turtle):
    ...
    def ausgabe(self):
        if self.speedwert == 6:
            self.write("Ergebnis: "+str(self.speedwert)+" HURRA!")
        else:
            self.write("Ergebnis: "+str(self.speedwert))
...
percent = screen.numinput("Window size",
                           "Prozent angeben:",
                           default=75, minval=50, maxval=100)
screen.setup(width = percent/100, height = percent/100)
...
root = tkinter.Tk()
width_pi = root.winfo_screenwidth() *percent/100
height_pi = root.winfo_screenheight() *percent/100
root.destroy()
